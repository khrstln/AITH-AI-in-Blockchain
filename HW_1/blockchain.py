...
class BlockchainPeer:
    ...
    @staticmethod
    def proof_of_work(block: Block, difficulty_bomb: bool = False) -> str:
        """
...
class BlockchainMainnet:
    ...
    def consensus(self):
        ...
        """
        logging.info("Mainnet | Consensus started")
        ...
        if not BlockchainPeer.check_chain_validity(longest_blockchain.chain):
            ...
            logging.info(
                f"""Mainnet | Consensus done with new chain {self.blockchain.peer_name} | Announcing new block {self.blockchain.last_block}"""  # noqa
            )
    ...
    def run_mining(self, difficulty_bomb: bool = False):
        """
        ...
        for peer in set(self.peers):
            tasks.append(threading.Thread(target=peer.mine, daemon=True, args=(difficulty_bomb,)))
